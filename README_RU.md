[English version](README.md)

# 🏨 Система бронирования отелей

**Hotel Booking System** — это полноценное backend-решение для бронирования отелей с REST API, выполняющим роль CMS-панели, системой управления пользователями, CSV-отчетностью и Kafka для обработки событий. Система поддерживает управление отелями, номерами, бронированиями, ролевой доступ и экспорт статистики через MongoDB.

---

## 🔍 Основной функционал

- Управление пользователями, ролями и авторизацией
- CRUD операции для отелей, номеров и бронирований
- Ролевая аутентификация (Admin/User) с **JWT Bearer Токенами**
- **Refresh Token** механизм для безопасного управления сессиями
- REST API выступает в роли CMS-панели
- Kafka интеграция для событий **UserRegistrationEvent** и **BookingEvent**
- Хранение статистики в MongoDB
- Экспорт статистики в CSV
- Использование DTO для всех запросов и ответов

---

## ⚙️ Технологии

- **Backend:** Java 21+, Spring Boot, Spring Data JPA, MapStruct, Validation
- **База данных (Polyglot Persistence):** PostgreSQL (основная), MongoDB (статистика), Redis (токены)
- **Миграции:** Liquibase
- **Месседжинг:** Kafka
- **Безопасность:** Spring Security, **JWT, Refresh Tokens**
- **Кэширование/Токены:** **Redis**
- **DevOps:** Docker, Docker Compose
- **Утилиты:** Lombok, Maven

---

## 🧩 Сущности

- **User, RoleType** — управление пользователями и правами (Роли хранятся через `@ElementCollection`)
- **RefreshToken** — сущность безопасности, управляемая Redis
- **Hotel, Room, Booking** — основные сущности бронирования
- **StatisticsDocument** — хранение статистики и генерация CSV

---

## 🧾 REST API Эндпоинты

### Аутентификация
| Метод | URL | Описание | Роли | DTO |
|-------|-----|----------|------|-----|
| POST | `/api/auth/register` | Регистрация нового пользователя | любой | RegisterRequest → LoginResponse |
| POST | `/api/auth/login` | Аутентификация и получение JWT/Refresh Токенов | любой | LoginRequest → LoginResponse |
| POST | `/api/auth/refresh-token` | Обновление JWT с использованием Refresh Token | любой | RefreshTokenRequest → RefreshTokenResponse |
| POST | `/api/auth/logout` | Деактивация Refresh Token и выход из системы | ADMIN, USER | — |

### Пользователи
| Метод | URL                  | Описание | Роли | DTO |
|-------|----------------------|----------|------|-----|
| GET | `/api/users`         | Получить всех пользователей | ADMIN | UserListResponse |
| GET | `/api/users/profile` | Получить профиль текущего пользователя | ADMIN, USER | String |
| GET | `/api/users/{id}`    | Получить пользователя по ID | ADMIN, USER | UserResponse |
| POST | `/api/users/create`  | Создать пользователя | любой | UserRequest → UserResponse |
| PUT | `/api/users/{id}`    | Обновить пользователя | ADMIN, USER | UserRequest → UserResponse |
| DELETE | `/api/users/{id}`    | Удалить пользователя | ADMIN, USER | — |

### Отели
| Метод | URL | Описание | Роли | DTO |
|-------|-----|----------|------|-----|
| GET | `/api/hotels` | Получить все отели | ADMIN | ListHotelResponse |
| GET | `/api/hotels/filter` | Фильтрация отелей | ADMIN, USER | HotelFilter → PageResponse<HotelResponse> |
| GET | `/api/hotels/{id}` | Получить отель по ID | ADMIN, USER | HotelResponse |
| POST | `/api/hotels` | Создать отель | ADMIN | HotelRequest → HotelResponse |
| PUT | `/api/hotels/{id}` | Обновить отель | ADMIN | HotelRequest → HotelResponse |
| DELETE | `/api/hotels/{id}` | Удалить отель | ADMIN | — |
| POST | `/api/hotels/review/{id}` | Добавить рейтинг | любой | RatingDTO → HotelResponse |

### Номера
| Метод | URL | Описание | Роли | DTO |
|-------|-----|----------|------|-----|
| GET | `/api/rooms/{id}` | Получить номер по ID | ADMIN, USER | RoomResponse |
| GET | `/api/rooms/filter` | Фильтрация номеров | ADMIN, USER | RoomFilter → PageResponse<RoomResponse> |
| POST | `/api/rooms` | Создать номер | ADMIN | RoomRequest → RoomResponse |
| PUT | `/api/rooms/{id}` | Обновить номер | ADMIN | RoomRequest → RoomResponse |
| DELETE | `/api/rooms/{id}` | Удалить номер | ADMIN | — |

### Бронирования
| Метод | URL | Описание | Роли | DTO |
|-------|-----|----------|------|-----|
| POST | `/api/bookings` | Создать бронирование | ADMIN, USER | BookingRequest → BookingResponse |
| GET | `/api/bookings` | Получить все бронирования | ADMIN | ListBookingResponse |

### Статистика
| Метод | URL | Описание | Роли | DTO |
|-------|-----|----------|------|-----|
| GET | `/statistics/csv` | Скачать CSV статистики | ADMIN | CSV файл (генерируется из StatisticsDocument) |

---

## 📦 Структура проекта

- **Controllers** — REST контроллеры для API, включая новый `AuthController`.
- **DTO** — классы запросов и ответов (User, Hotel, Room, Booking, Statistics, Errors, **Auth**)
- **Services** — бизнес-логика и обработка событий, включая `AuthService` и `RefreshTokenService`.
- **Mappers** — MapStruct мапперы для конвертации DTO
- **Repositories** — Spring Data JPA / Mongo / **Redis** репозитории
- **Security** — аутентификация, JWT-компоненты и ролевой доступ
- **Kafka** — продюсеры и консьюмеры для событий пользователей и бронирований

---

## ⚙️ Конфигурация

Приложение использует **application.yaml** для настройки сервера, баз данных, Kafka и параметров безопасности.

Пример `application.yaml`:

```yaml
server:
  port: 8080

spring:
  datasource:
    url: jdbc:postgresql://localhost:5432/hotel
    username: postgres
    password: password
  jpa:
    hibernate:
      ddl-auto: validate # Сменили с update на validate для безопасности
    show-sql: true
  liquibase:
    change-log: classpath:db/changelog/db.changelog-master.yml
    enabled: true
    drop-first: true
  data:
    mongodb:
      host: localhost
      port: 27017
      database: statistics_db
    redis: # <-- НОВОЕ
      host: localhost # <-- НОВОЕ
      port: 6379 # <-- НОВОЕ

kafka:
  bootstrap-servers: localhost:9092

app:
  kafka:
    statisticsTopic: "statistics"
    kafkaMessageGroupId: "kafka-message-group-id"
  jwt:
    secret: "yourSecretKey" # <-- НОВОЕ
    tokenExpiration: 12h # <-- НОВОЕ
    refreshTokenExpiration: 24h # <-- НОВОЕ
```
## 🔑 Основные параметры

- `server.port` — порт приложения
- `spring.datasource.*` — настройки подключения к PostgreSQL
- `spring.jpa.hibernate.ddl-auto` — управление схемой базы данных
- `kafka.*` — Kafka topics и bootstrap server
- `mongodb.uri` — MongoDB для хранения статистики
- `redis.*` — Подключение Redis для Refresh Токенов
- `app.jwt.*` — Секрет JWT и время истечения токенов

---

## 🚀 Локальный запуск

### Требования

- Java 21+
- Maven
- PostgreSQL
- MongoDB
- Kafka
- Redis
- Docker & Docker Compose (опционально)

### Шаги запуска

1. Клонировать репозиторий:

```bash
git clone <REPO_URL>
cd <PROJECT_FOLDER>
```
### Настройка и запуск

1. Настроить PostgreSQL, MongoDB и Kafka в `application.yaml`.

```bash
cd docker
docker compose up
cd ..
```

2. Собрать и запустить проект:

```bash
mvn clean install
mvn spring-boot:run
```
### Доступ к API

[http://localhost:8080](http://localhost:8080)

---

### ⚡ Примечания архитектуры

* **Контроль структуры БД** — миграции полностью управляются через **Liquibase**. Прямое изменение схемы через Hibernate отключено (`ddl-auto: validate`), что гарантирует сохранность данных.
* **Интерактивная документация** — полное описание API доступно через **Swagger UI** (OpenAPI 3.1) с поддержкой тестирования авторизации через JWT.
* **Event-Driven подход** — критические события (регистрация, бронирование) асинхронно передаются через **Kafka** для последующей аналитики.
* **Статистика и аналитика** — данные агрегируются в **MongoDB**, что позволяет разгрузить основную БД и быстро генерировать CSV-отчеты.
* **Безопасность сессий** — механизм **Refresh Tokens** реализован на базе **Redis**, что обеспечивает мгновенную проверку и отзыв токенов без лишних запросов к основной БД.
* **Валидация и DTO** — строгий контракт данных; все входящие объекты проходят проверку `jakarta.validation`.